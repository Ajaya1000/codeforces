...
void pre() {
    inv[0] = inv[1] = 1;
    rep(i,2,2*N-1) inv[i] = inv[mod%i] * (mod - (mod/i));

    cat[0] = cat[1] = 1;
    rep(i,2,N-1) cat[i] = cat[i-1] * inv[i+1] * (ll)(4*i-2);

    inv_cat[0] = inv_cat[1] = 1;
    rep(i,2,N-1) inv_cat[i] = inv_cat[i-1] * inv[2] * inv[2*i-1] * (ll)(i+1);
}
